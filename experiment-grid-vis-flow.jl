...
function getflow(A,x)
    ei,ej = findnz(triu(A,1))[1:2]
    vals = zeros(length(ei))
    for nz=1:length(ei)
        src = ei[nz]
        dst = ej[nz]
        vals[nz]= abs(x[src]-x[dst])
    end
    return vals
end
function myflowgraph(A,xy,x;threshhold=0,nlevels=4)
    f = getflow(A,x)
    minf,maxf = extrema((f[f.>threshhold]))
    logflow = map( x -> x > threshhold ? log10(x) : Inf, f)

    # now convert flow to contour
    nzset = f .> threshhold
    levels = quantile(f[nzset], range(0, 1-1/nlevels, length=nlevels))
    pushfirst!(levels, 0.0)

    #

    draw_graph(A,xy;
    linewidth=0.75, color=1, markersize=0.5, markerstrokewidth=0,
    markercolor=:lightblue, markerstrokecolor=:white, framestyle=:none,
    margin=-20mm, legend=false, marker=:dot, size=(150,150), dpi=300,
    xlims=(4.5, 45.5), ylims=(4.5, 45.5),
        line_z = logflow, clim = (log10(minf),log10(maxf)))
end
myflowgraph(A,xy,x)
...
## test out coords for getflow
function test_flowcoords(A,xy,x)
    myflowgraph(A,xy,x)
    f = getflow(A,x)
    ei,ej = findnz(triu(A,1))[1:2]
    xyf = zeros(length(ei),2)
    for nz=1:length(ei)
        src = ei[nz]
        dst = ej[nz]
        xyf[nz,1]= 0.5*xy[src,1]+0.5*xy[dst,1]
        xyf[nz,2]= 0.5*xy[src,2]+0.5*xy[dst,2]
    end

    scatter(xyf[:,1],xyf[:,2],marker_z=log10.(f),markersize=2,markerstrokewidth=0)
end
test_flowcoords(A,xy,x)

## Okay, we are going to have to write a bunch of code to get the
# right grid flow contours. We are first going to interpolate the flow
# to a regular grid.
function flow_on_grid(A,xy,x)
    ei,ej = findnz(triu(A,1))[1:2]
    xyf = zeros(length(ei),2)
    ...
    for nz=1:length(ei)
        src = ei[nz]
        dst = ej[nz]
        xval = 0.5*xy[src,1]+0.5*xy[dst,1]
        yval = 0.5*xy[src,2]+0.5*xy[dst,2]
        vals[xval,yval] = abs(x[src]-x[dst])
        push!(xf,xval)
        push!(yf,yval)
    end
    xf = sort(collect(xf))
    yf = sort(collect(yf))
    F = zeros(length(xf),length(yf))
        ...
        for (j,y) in enumerate(yf)
            if haskey(vals,(x,y))
                ...
                if x == xmin || x == xmax || y == ymin || y == ymax
                    ...
                    F[i,j] = vals[xf[i-1],y]*0.25 + vals[xf[i+1],y]*0.25 +
                                vals[x,yf[j-1]]*0.25 + vals[x,yf[j+1]]*0.25
                    #F[i,j] = max(vals[xf[i-1],y]*0.5 + vals[xf[i+1],y]*0.5,
                    #             vals[x,yf[j-1]]*0.5 + vals[x,yf[j+1]]*0.5)
                    #vmax = max(vals[xf[i-1],y],vals[xf[i+1],y],vals[x,yf[j-1]],vals[x,yf[j+1]])
                    #vmin = min(vals[xf[i-1],y],vals[xf[i+1],y],vals[x,yf[j-1]],vals[x,yf[j+1]])
                    #F[i,j] = vmax
                end
            end
        end
    ...
    return sort(collect(xf)),sort(collect(yf)),F
end
xf,yf,F=flow_on_grid(A,xy,x)
heatmap(log10.(F))
##
function flow_on_grid(A,xy,x)
    ei,ej = findnz(triu(A,1))[1:2]
    ...
    for nz=1:length(ei)
        ...
        f = abs(x[src]-x[dst])
        vals[xval,yval] = f
        vals[xy[src,1],yval] = 0.25*f
        vals[xy[dst,1],yval] = 0.25*f
        vals[xval,xy[src,2]] = 0.25*f
        vals[xval,xy[dst,2]] = 0.25*f
        push!(xf,xval)
        push!(xf,xy[src,1])
        push!(xf,xy[src,2])
        push!(yf,yval)
...
@time x = SLQ.slq_diffusion(SLQ.graph(A), [seed], 0.0001, 0.0001, 0.99,
    SLQ.QHuberLoss(5.0, 0.0); max_iters=100000000)[1]
myflowgraph(A,xy,x; threshhold=0)
